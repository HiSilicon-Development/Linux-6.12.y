...
static MXL_STATUS_E MxL_HRCLS_Ctrl_ReadXptField(UINT8 devId, UINT32 regAddr,
                                                UINT8 lsbPos, UINT8 fieldWidth,
                                                /*@out@*/ UINT32 *valuePtr)
{
  UINT8  status;
  UINT32 regData = 0;
...
{
	static const MXL_HRCLS_FIELD_T mpeg_oe[] = {
		{ MPEG0_OE }, { MPEG1_OE }, { MPEG2_OE }, { MPEG3_OE },
		{ MPEG4_OE }, { MPEG5_OE }, { MPEG6_OE }, { MPEG7_OE },
		{ MPEG8_OE },
	};
	static const MXL_HRCLS_FIELD_T pad_sync[] = {
		{ PAD_MPEG_1_SYN_DRV }, { PAD_MPEG_2_SYN_DRV },
		{ PAD_MPEG_3_SYN_DRV }, { PAD_MPEG_4_SYN_DRV },
		{ PAD_MPEG_5_SYN_DRV }, { PAD_MPEG_6_SYN_DRV },
		{ PAD_MPEG_7_SYN_DRV }, { PAD_MPEG_8_SYN_DRV },
	};
	static const MXL_HRCLS_FIELD_T pad_data[] = {
		{ PAD_MPEG_1_DAT_DRV }, { PAD_MPEG_2_DAT_DRV },
		{ PAD_MPEG_3_DAT_DRV }, { PAD_MPEG_4_DAT_DRV },
		{ PAD_MPEG_5_DAT_DRV }, { PAD_MPEG_6_DAT_DRV },
		{ PAD_MPEG_7_DAT_DRV }, { PAD_MPEG_8_DAT_DRV },
	};
	static const MXL_HRCLS_FIELD_T pad_valid[] = {
		{ PAD_MPEG_1_VAL_DRV }, { PAD_MPEG_2_VAL_DRV },
	...
	MXL_HRCLS_XPT_OUTPUT_ID_E physical_output;
	UINT32 ts_clock_register = 0;
	UINT8 status = MXL_SUCCESS;

	if (!readback || logical_output >= MXL214_NUM_FRONTENDS)
		return -EINVAL;

	context = MxL_HRCLS_Ctrl_GetDeviceContext(device_id);
	physical_output = MxLWare_HRCLS_Ctrl_GetPhysicalXptOutputId(
		context, (MXL_HRCLS_XPT_OUTPUT_ID_E)logical_output);
	if (physical_output == MXL_HRCLS_XPT_OUT_INVALID ||
	    (UINT8)physical_output >= ARRAY_SIZE(pad_sync))
		return -EINVAL;

	*readback = (struct mxl214_xpt_readback) {
		.physical_output = physical_output,
	};
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30000,
		&readback->input_enable);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30008,
		&readback->mux_mode);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x3000c,
		&readback->output_enable_raw);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30010,
		&readback->clock_polarity);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30014,
		&readback->valid_polarity);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30184,
		&readback->mode_27mhz);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30190,
		&readback->pid_mux0);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x301b0,
		&readback->pid_mux1);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x301d4,
		&readback->ts_clock_enable_raw);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x30238,
		&readback->nco_1_4);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id, 0x3023c,
		&readback->nco_5_7);
	status |= MxL_HRCLS_Ctrl_ReadXptField(device_id,
		enable[(UINT8)physical_output].regAddr,
		enable[(UINT8)physical_output].lsbPos,
		enable[(UINT8)physical_output].fieldWidth,
		&readback->output_enable);
	status |= MxL_HRCLS_Ctrl_ReadXptField(device_id,
		ncoCountMin[(UINT8)physical_output].regAddr,
		ncoCountMin[(UINT8)physical_output].lsbPos,
		ncoCountMin[(UINT8)physical_output].fieldWidth,
		&readback->nco_count_min);
	status |= MxL_HRCLS_Ctrl_ReadXptRegister(device_id,
		EXTRACT_ADDR32(TS_CLK_OUT_EN0), &ts_clock_register);
	readback->ts_clock_enable =
		!!(ts_clock_register & (1U << physical_output));
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id,
		mpeg_oe[(UINT8)physical_output + 1].regAddr,
		mpeg_oe[(UINT8)physical_output + 1].lsbPos,
		mpeg_oe[(UINT8)physical_output + 1].fieldWidth,
		&readback->mpeg_output_enable);
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id, MCK_OE,
		&readback->mpeg_clock_enable);
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id, XPT_TS_MODE,
		&readback->xpt_ts_mode);
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id,
		pad_sync[(UINT8)physical_output].regAddr,
		pad_sync[(UINT8)physical_output].lsbPos,
		pad_sync[(UINT8)physical_output].fieldWidth,
		&readback->pad_sync_drive);
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id,
		pad_data[(UINT8)physical_output].regAddr,
		pad_data[(UINT8)physical_output].lsbPos,
		pad_data[(UINT8)physical_output].fieldWidth,
		&readback->pad_data_drive);
	status |= MxL_HRCLS_Ctrl_ReadRegisterField(device_id,
		pad_valid[(UINT8)physical_output].regAddr,
		pad_valid[(UINT8)physical_output].lsbPos,
		pad_valid[(UINT8)physical_output].fieldWidth,
		&readback->pad_valid_drive);
	status |= MxLWare_HRCLS_OEM_ReadRegister(device_id,
		EXTRACT_ADDR(ECO_4), &readback->eco4);
	status |= MxLWare_HRCLS_OEM_ReadRegister(device_id,
		EXTRACT_ADDR(ECO_8), &readback->eco8);

	return status == MXL_SUCCESS ? 0 : -EIO;
}
