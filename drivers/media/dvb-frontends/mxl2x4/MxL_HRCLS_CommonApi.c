  ...
  if (devContextPtr)
  {
    ...
    // Reset MxL_HRCLS by toggling Reset pin
      ...
      if (status == MXL_SUCCESS)
      {
        ...
        UINT16 flavorId = 0;
        UINT8 flavorReadStatus;

        flavorReadStatus = MxL_HRCLS_Ctrl_ReadRegisterField(devId, ANA_DIG_SOC_FLVR, &flavorId);
        status |= flavorReadStatus;
        flavorId >>= 4;
        flavorId &= 0x0f;
        ...
        {
          if (flavorReadStatus == MXL_SUCCESS)
          {
            MXLERR(MxL_HRCLS_ERROR("[MXL][HRCLS][%d] Device flavor 0x%x does not match requested type %d\n",
                                   devId, flavorId, devContextPtr->deviceType);)
          }
          else
          {
            MXLERR(MxL_HRCLS_ERROR("[MXL][HRCLS][%d] Unable to read device flavor: status=%u\n",
                                   devId, flavorReadStatus);)
          }
        }
#endif
      }
    ...
    )
{
  ...
  if ((devContextPtr) && (mbinBufferPtr))
  {
    if (devContextPtr->driverInitialized)
    {
      ...
      if ((status == MXL_SUCCESS) && ((status = MxL_HRCLS_Ctrl_DownloadFirmwareInMbin(devContextPtr, (MBIN_FILE_T*)(mbinBufferPtr), mbinBufferSize, MXL_ENABLE, fwCallbackFn)) == MXL_SUCCESS))
      {
