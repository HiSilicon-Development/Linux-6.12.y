...
static MXL_STATUS_E MxL_HRCLS_Ctrl_DownloadFirmwareSegment(MXL_HRCLS_DEV_CONTEXT_T* devContextPtr, UINT8 segmentIndex, const UINT8 * addressPtr, const UINT8* dataPtr, UINT32 dataLen, /*@null@*/ MXL_CALLBACK_FN_T fwCallbackFn);
...
MXL_STATUS_E MxL_HRCLS_Ctrl_DownloadFirmwareInMbin(MXL_HRCLS_DEV_CONTEXT_T* devContextPtr, const MBIN_FILE_T* mbinPtr, UINT32 mbinSize, UINT8 enableRun, MXL_CALLBACK_FN_T fwCallbackFn)
{
  const UINT8 *mbinData = (const UINT8 *)mbinPtr;
  const MBIN_SEGMENT_T *segmentPtr;
  UINT32 index;
  UINT32 segLength;
  ...
  if ((!devContextPtr) || (!mbinPtr) || (mbinSize < sizeof(MBIN_FILE_HEADER_T)))
  {
  ...
  imageLength = (((UINT32)mbinPtr->header.imageSize24[0]) << 16)
              | (((UINT32)mbinPtr->header.imageSize24[1]) << 8)
              | ((UINT32)mbinPtr->header.imageSize24[2]);
  if ((mbinPtr->header.id == MBIN_FILE_HEADER_ID)
      && (mbinPtr->header.fmtVersion == MBIN_FORMAT_VERSION)
      && (mbinPtr->header.headerLen >= sizeof(MBIN_FILE_HEADER_T))
      && (mbinPtr->header.headerLen <= mbinSize)
      && (mbinPtr->header.numSegments > 0)
      && (imageLength == mbinSize - mbinPtr->header.headerLen))
  {
    offset = mbinPtr->header.headerLen;
    /* Process each segment */
    for (index = 0; ((index < mbinPtr->header.numSegments) && (status == MXL_SUCCESS)); index++)
    {
      if ((offset > mbinSize)
          || (sizeof(MBIN_SEGMENT_HEADER_T) > mbinSize - offset))
      {
        status = MXL_FAILURE;
        break;
      }
      segmentPtr = (const MBIN_SEGMENT_T *)(mbinData + offset);
      ...
      if (segmentPtr->header.id == MBIN_SEGMENT_HEADER_ID)
      {
        ...
        alignedLength = (segLength + 3) & ~3U;
        if (alignedLength > mbinSize - offset - sizeof(MBIN_SEGMENT_HEADER_T))
        {
          status = MXL_FAILURE;
          break;
        }
        ...
        {
          /* Next segment */
          offset += sizeof(MBIN_SEGMENT_HEADER_T) + alignedLength;
        }
      }
      else
      {
    ...
    if ((status == MXL_SUCCESS) && (offset != mbinSize))
    {
      status = MXL_FAILURE;
    }
...
static MXL_STATUS_E MxL_HRCLS_Ctrl_DownloadFirmwareSegment(MXL_HRCLS_DEV_CONTEXT_T* devContextPtr, UINT8 segmentIndex, const UINT8 * addressPtr, const UINT8* dataPtr, UINT32 dataLen, MXL_CALLBACK_FN_T fwCallbackFn)
{
  ...
  if (status == MXL_SUCCESS)
  {
    while ((status == MXL_SUCCESS) && (dataLen>0))
    {
      ...
      status = MxL_HRCLS_Ctrl_SendDownloadCommand(devContextPtr, HOST_CMD_CFG_DOWNLOAD_SEGMENT, dataPtr, nSend, seqNum);
      if (status == MXL_SUCCESS)
      {
...
static __maybe_unused MXL_STATUS_E MxL_HRCLS_Ctrl_GetVbeCode(
   UINT8 devId,
   UINT8 tsensMode,
   UINT16* vbeCode
   )
